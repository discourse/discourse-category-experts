...
class ChangeBooleanCustomFieldsToInts < ActiveRecord::Migration[6.1]
  ...
  def set_topic_needs_approval_to_int
    topic_needs_approval_custom_field_ids =
      execute(
        "SELECT topic_id FROM topic_custom_fields WHERE name='category_expert_topic_post_needs_approval' AND value = 't'",
      ).values.flatten
  ...
  def set_topic_first_approved_post_ids
    topic_approved_custom_field_ids =
      execute(
        "SELECT topic_id FROM topic_custom_fields WHERE name='category_expert_topic_approved_group_names' AND value IS NOT NULL",
      )
